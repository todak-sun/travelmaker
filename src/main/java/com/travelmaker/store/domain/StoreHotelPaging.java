...
public class StoreHotelPaging {
	...
	public void makePagingHTML() {
		...
		if (startPage > pageBlock) {
			pagingHTML.append("<li class='page-item'><a class='page-link' id='paging' href='/store/list/"
					+ (startPage - 1) + "'>이전</a></li>");
		}

		for (int i = startPage; i <= endPage; i++) {
			if (i == currentPage) {
				pagingHTML.append("<li class='page-item active'><a class='page-link' id='paging' href='/store/list/"
						+ i + "'>" + i + "</a></li>");
			} else {
				pagingHTML.append("<li class='page-item'><a class='page-link' id='paging' href='/store/list/" + i
						+ "'>" + i + "</a></li>");
			}
		}
		if (endPage < totalP) {
