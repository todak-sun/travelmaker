$(function () {
                ...
                addEvent(btnSearch, 'click', function (e) {
                    ...
                    ajax
                        ...
                        .then((ret) => {
                            ...
                            if (!ret.length)
                                $messageBox.append(`<p>조회된 아이디가 없습니다.</p>`);
                            else {
                                $messageBox.append(
                                    `<p>총 ${ret.length}개의 아이디가 조회되었습니다.</p>`
                                );
                    ...
                    ajax
                        ...
                        .then((ret) => {
                            ...
                            if (ret)
                                ...
                                $messageBox.append(
                                    `<p>존재하지 않는 아이디 또는 가입시 입력한 이메일이 아닙니다.</p>`
                                );
                            closeLoading();
                        })
                        .catch(console.error);
    ...
    function initRegisterModal() {
        ...
        addAllSameEvent(
            customRadios,
            'click',
            h.customRadioHandler.bind(null, customRadios)
        );
        ...
        function emailCheckHandler() {
            ...
            showLoading();
            ajax
                .sendEmailCode(email1.value, email2.value)
                .then((ret) => {
                    closeLoading();
                    miniModal.createMini(
                        t.emailConfirm(),
                        h.initEmailConfirmModal.bind(
                            null,
                            btnEmailCheck,
                            ret.emailCode,
                            email1,
        ...
        function nicknameHandler(e) {
            ...
            const [vFeed, ivFeed] = v.getFeedBox(this);
            if (!this.value)
                return v.setInvalid(this, ivFeed, '닉네임을 입력해주세요.');
            if (!regex.test(this.value))
                return v.setInvalid(
                    this,
                    ivFeed,
                    '한글 최대 10자, 영문 20자로 한글, 영문, 숫자만 사용 가능합니다.'
                );
            ajax
                ...
                .then((ret) => {
                    ...
                    if (ret === 'not_exist')
                        return v.setValid(e.target, vFeed, '사용 가능한 닉네임입니다.');
                })
                .catch(console.error);
        }
        ...
        function btnUserHandler(e) {
            ...
            if (!v.isValid(btnEmailCheck)) return alert('이메일 인증을 진행해주세요');
    ...
    if (seq) {
        ...
        ajax.alarmDataLoad(seq.value).then((ret) => {
        ...
        function printMessage(alarmList) {
            ...
            if (!alarmList.length) {
                ...
                addAllSameEvent(checkList, 'click', function (e) {
                    ...
                    ajax
                        .checkAlarm(header, ano)
                        .then(({fno}) => {
                            if (header === 'friend')
                                return (location.href = `/${header}/view/${fno}`);
                            if (header === 'purA')
                                return (location.href = `/pur/view/1/${fno}`);
                            if (header === 'purB')
                                return (location.href = `/pur/view/2/${fno}`);
                            if (header === 'cash')
            ...
            addAllSameEvent(closeList, 'click', (e) =>
                $(e.target)
                    .closest('li')[0]
                    .remove()
            );
