...
$(function () {
    ...
    const {getEl, setRequestHeader, setInValid, setValid, changeValid, changeInValid, isValid, getRegisterMethod, getFeedbackBox, resetMessageHandler} = new travelmaker.utils();
    ...
    function setTitleAndBody(id) {
        ...
        switch (id) {
            ...
            case 'btn-login':
                ...
                return ['로그인', template.login(csrfTokenName, csrfTokenValue), initLoginSelector];
            case 'btn-regist':
                return ['회원가입', template.register(csrfTokenName, csrfTokenValue), initRegisterSelector];
            default:
    ...
    function initRegisterSelector() {
        ...
        function phoneHandler(e) {
            ...
            if (!regex.test(value)) return setInValid(e, inFeedback, '숫자가 아닌 값은 입력할 수 없습니다.');
            changeValid(e);
        }
        ...
        function emailHandler(e) {
            ...
            if (!regex.test(value)) return setInValid(e, inFeedback, '정확한 이메일을 입력해주세요.');
            changeValid(e);
        }
        ...
        function realnameHandler(e) {
            ...
            if (!regex.test(realname)) return setInValid(e, inFeedback, '2 ~ 5글자의 정확한 이름을 입력해주세요.');
            changeValid(e);
        }
        ...
        function registerPasswordHandler(e) {
            ...
            if (!regex.test(password)) return setInValid(e, inFeedback, '특수문자 / 문자 / 숫자 포함 형태의 8~15자리 비밀번호로 설정해주세요');
        ...
        function registerRepasswordHandler(e) {
            ...
            if (pwd !== repwd) return setInValid(e, inFeedback, '동일한 비밀번호를 입력해주세요.');
            if (!isValid(registerPassword)) return setInValid(e, inFeedback, '유효한 비밀번호 설정후 다시 시도해주세요');
            setValid(e, vaFeedback, '동일한 비밀번호를 입력하셨습니다.');
        }

        function userAgreeHandler(e) {
            userAgreeBoxAll.checked = document.querySelectorAll('.user-agree-box:checked').length === 3;
        }
        ...
        function registerIdHandler(e) {
            ...
            if (!regex.test(id)) return setInValid(e, inFeedback, '아이디는 이메일 형식으로 입력해주세요.');
            ajaxCheckId({id: id, registerMethod: getRegisterMethod()})
                .then(ret => {
                    if (ret === 'exist') {
                        return setInValid(e, inFeedback, '이미 사용중인 아이디입니다. 다시 입력해주세요.');
                    }
