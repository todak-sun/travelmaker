...
function naverLogin() {
	...
	let name = $('#naverId').val();
	let id = $('#naverEmail').val();
	...
	$.ajax({
		type : 'post',
		url : './user/registerMethod',
		data : 'name=' + name + '&id=' + id + '&registerMethod=naver',
		dataType : 'text',
		beforeSend : function(xhr) {
		...
		success : function(data) {
			if (data == 'null') {
				...
				var Link = document.getElementById('btn-regist');
				var event = document.createEvent("MouseEvents");
				event.initEvent("click", false, true);
				Link.dispatchEvent(event);

				$('#registerMethod').val('naver');
				$('#realname').val(name);
				$('#registerId').val(id);
				$('#registerId').attr("readonly", true); // 설정
...
function onSignIn(googleUser) {
	...
	$.ajax({
		type : 'post',
		url : './user/registerMethod',
		data : 'name=' + name + '&id=' + id + '&registerMethod=google',
		dataType : 'text',
		beforeSend : function(xhr) {
			xhr.setRequestHeader(header, token);
		},
...
$('#prePassword')
		.focusout(
				function() {
					...
					if ($('#prePassword').val() == '') {
					...
					} else if (!passRule.test($("input[id='prePassword']")
							.val())) {
						$('#prePasswordDiv').text(
						...
						$.ajax({
							type : 'post',
							url : './checkPassword',
							data : 'id=' + $('#mypageId').val()
									+ '&registerMethod='
									+ $('#registerMethod').val() + '&pwd='
									+ $('#prePassword').val(),
							dataType : 'text',
							beforeSend : function(xhr) {
								// here it is
...
$('#nextPassword')
		.focusout(
				function() {
					...
					} else if (!passRule.test($("input[id='nextPassword']")
							.val())) {
