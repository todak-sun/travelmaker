...
$(function(){
	var $isDomestic = $('#is_domestic').val();
	...
	if($isDomestic == 1) {
     ...
     // 해외 (Google Map)
		...
		// 카카오 지도 모달 연결 속성 없애기
		$searchPlaceBtn.removeAttr('data-toggle');
		$searchPlaceBtn.removeAttr('data-target');
	}
});
...
$('#context').focusout(function(){
	...
	if(context == '') {
		contextDiv.css('font-size', '12px').css('font-style', 'bold').css('color', 'red').text('내용을 입력해주세요.');
	} else {
		contextDiv.removeAttr('style');
		contextDiv.empty();
	}
});
...
$('#kakaoMapModal').on('shown.bs.modal', function() {
	$('#keyword').val($('#searchPlace').val()); // 검색 키워드 가져오기
	// 마커를 담을 배열
	markers = [];
...
function myMap() {
	...
	searchBox.addListener('places_changed', function() {
		...
		places.forEach(function(place) {
			...
			// 마커 아이콘 생성 (없으면 기본 마커)
			/*
			 * var icon = { url: place.icon, size: new google.maps.Size(71, 71),
			 * origin: new google.maps.Point(0, 0), anchor: new
			 * google.maps.Point(17, 34), scaledSize: new google.maps.Size(25,
			 * 25) };
			 */
		...
		$('#searchPlace').val($('#pac-input').val());
		$('#city').val($('#pac-input').val());
