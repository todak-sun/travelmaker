$(function () {
    ...
    document
        .querySelector(".title-expression")
    ...
    function checkValidation(data, isDomestic) {
        const {content, dateStart, dateEnd, place} = data;

        if (!checkPlaceValidation(place)) return false;
        if (!checkContentValidation(content)) return false;
        if (!checkDateValidation(dateStart, dateEnd)) return false;
        return true;
    }
    ...
    function checkDateValidation(dateStart, dateEnd) {
        let date = new Date();
        let year = date.getFullYear();
        let month = date.getMonth() + 1;
        let day =
        ...
        if (!dateStart || !dateEnd) {
            ...
            alert("시작날짜가 종료날짜보다 뒤에 있을 수 없습니다");
            dateStart.focus();
            return false;
        } else if (dateEnd > today) {
            alert("미래날짜는 입력하실 수 없습니다");
            dateStart.focus();
            return false;
        }
    ...
    function checkMainImageValidation(value) {
        let imageMain = getEl("#img-main");
        // if (imageMain == null) {
        //   alert("대표 이미지를 선택해주세요");
        //   return false;
        // } else
        if (!value && imageMain.clientHeight == 0) {
            alert("대표 이미지를 선택해주세요");
            return false;
        }
